...
export default function ClockDifficulty() {
  return (
    <S.ProgramSection className="bg-suabg-1 bg-contain">
      <S.ProgramBox>
        <S.ProgramHeader>🕑시계 읽기</S.ProgramHeader>
        <S.DiffSection>
          <S.DiffLink to="/ClockProgram" state={{ difficulty: 'high' }}>
            <S.DiffH1>상</S.DiffH1>
          </S.DiffLink>
          <S.DiffLink to="/ClockProgram" state={{ difficulty: 'middle' }}>
            <S.DiffH1>중</S.DiffH1>
          </S.DiffLink>
...
const S = {
  ProgramSection: styled.div`
    ${tw`flex bg-brand min-h-screen mt-[90px] justify-center items-center`}
  `,
